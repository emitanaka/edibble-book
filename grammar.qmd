...
[Chapter -@sec-basics] described a number of basic terms and concepts in experimental designs. In this chapter, I dissect these basic terms in order to build the "grammar of experimental designs". 
...
In the grammar, a **factor** is an experimental variable manifested as an object with a dual structure, the "root" and the "head". The **head** characterizes the holistic attributes of the factor (e.g. the label "vaccine" for the treatment factor) and the **root** is the constitution of the discrete factor levels such that it captures the idiosyncratic semantic features of each level that distinguish itself from other levels (e.g. labels "type A" and "type B" for the two types of vaccine treatment). Every root and head structure has an outer form that is visible to the user (usually in the form of a vertex or a vector) and an inner quality that are intrinsic characteristics of the outer form (e.g. id of the levels, the long name of the vaccines, dose level of each vaccine, etc). The inner quality are not usually exported in the final design table. In the system, every root structure co-exists with a head structure in a reciprocal relationship, i.e. any change in the root structure may be reflected as a change in head structure (and vice-versa). A schema of the factor structure is shown in @fig-factor-structure.

![A schema for the factor structure in the grammar.](figures/drawn/factor-structure){#fig-factor-structure}

Every factor in the system is given an **explicit role** stored as a class in the inner quality of the head. The main roles are unit, treatment and record. The relationship between factors assigns an implicit role, e.g. treatment linked as a directed graph to a unit as seen in @fig-implicit-inner-quality.


![Implicit role based on relationship between factors](figures/drawn/implicit-inner-quality){#fig-implicit-inner-quality}

In building an experimental design, the system tasks factors to link the nodes in the root with some constraint as subscribed by their explicit role.
