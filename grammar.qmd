...
In the grammar, a **factor** is an experimental variable manifested as an object with a dual structure: the "head" and the "root". The **head** characterizes the holistic attributes of the factor (e.g. the label "vaccine" for the treatment factor) and the **root** is the constitution of the discrete factor levels such that it captures the idiosyncratic semantic features of each level that distinguish itself from other levels (e.g. labels "type A" and "type B" for the two types of vaccine treatment). Every root and head structure has an outer form that is visible to the user (usually in the form of a vertex or a vector) and an inner quality that are intrinsic characteristics of the outer form (e.g. id of the levels, the long name of the vaccines, dose level of each vaccine, etc). A factor can be physical (e.g. person, plot, animal and drug),  metaphysical (e.g. gender, time and space) or intended response measures. The inner quality are not usually exported in the final design table. In the system, every root structure co-exists with a head structure in a reciprocal relationship, i.e. any change in the root structure may be reflected as a change in head structure (and vice-versa). A schema of the factor structure is shown in @fig-factor-structure.
...
record | unit | B is measured on A | Obervational unit
