...
In the grammar, a **factor** is an experimental variable manifested as an object with a dual structure, the "root" and the "role". The **root** is the constitution of the discrete factor levels and the **head** characterizes the holistic attributes of the factor. For example, consider an agricultural field trial with plot as a unit factor -- the root is the plots and the head corresponds to the whole set of plots. Every root and head structure has an outer form that is visible to the user (usually in the form of a graph) and an inner quality that are intrinsic characteristics of the outer form. E.g., the dimension of the plot size may be described in the head as an inner quality (i.e. not something that is readily visible to the user but stored as a meta information about the factor). In addition, every root structure co-exists with a head structure in a reciprocal relationship. 
