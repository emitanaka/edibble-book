...
Record is used here to mean any uncontrollable factor. These can include the response, nuisance factor and just any value that is observed. 

* **Observation** implies the observed value of a single unit.
* **Response** is usually the observation of interest in an experiment. 
* **Dependent variable** is another way to call response.
* **Nuisance factor** 
* **Lurking variable**
* **Trait**


Records are captured at different time points. We can map the time point categorical relative to the experiment: historical (pre-experiment), pre-treatment, post-treatment, followup/future (assuming measured once), or even the frequency of it: daily, weekly, monthly etc. 

```{r}
#| eval: false
set_rcrds(yield = measure(plot, unit = "t/h", when = "pre"))
```
