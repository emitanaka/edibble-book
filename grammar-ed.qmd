...
## Rule `r count_rule()`: Explicit roles are encoded as a class

All experimental factors must be encoded as an object with the factor class (e.g. `edbl_fct`) with additional expansible explicit roles as unit (`edbl_unit`), treatment (`edbl_trt`), record (`edbl_rcrd`) or other. Roles determined implicitly by relationship with other factors should not be encoded as a class. 
...
In the system, every root system co-exists with a head system in a reciprocal relationship, i.e. any change in the root structure may be reflected as a change in head structure (and vice-versa). A schema of the factor structure is shown in @fig-factor-structure.
...
## Rule `r count_rule()`: Dual structure of factors
...
## Rule `r count_rule()`: Rules for relationships between factors
...
![An example of the full schema of an experimental design with 3 factors: diet, subject, and day. There are three types of diet (Atkins, Keto, Vegan and Paleo), 4 different subjects per day over a total of two days.](figures/drawn/example-factor-structure){#fig-example-network}
...
## Rule `r count_rule()`: All factor levels must map to a single observational unit
