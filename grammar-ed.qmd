...
a) A higher-order link between any two factor nodes must be explicitly specified by the user.
b) A lower-order link between any two factor nodes cannot be made if there is no higher-order link between the factor nodes linked to the two level nodes. 
