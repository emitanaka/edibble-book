...
In a comparative experiment, treatments are allocated to units but every unit has an individual variation (with exceptions for some, e.g. computer experiments). As a simple example, consider @fig-replication.


```{r fig-replication, echo = FALSE, fig.cap = "A comparative experiment with two treatments allocated to four participants. Each treatment is replicated twice with the same outcome for the same treatment.", out.width="100%"}
