...
In some software, $\mathbf{V}$ may (inadvertentnly or not) allow positive semi-definite matrix. This causes issues when $\mathbf{V}$ becomes singular or when one of the eigenvalues of $\mathbf{V}$ is near-zero and there are some strategies that the software employs to tackle these issues.
