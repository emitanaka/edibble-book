...
Most optimal-criteria is based on optimising some function based on  $(\mathbf{X}^\top\mathbf{X})^{-1}$ or $\mathbf{X}^\top\mathbf{X}$, also known as the **information matrix** (although to be more specific, the actual information matrix corresponds to $\sigma^{-2}\mathbf{X}^\top\mathbf{X}$). The list of optimal-criteria are shown in Table XX.

| Criteria | Description |
| --- | --- |
| A-optimality  |  minimise $\text{tr}\left((\mathbf{X}^\top\mathbf{X})^{-1}\right)$ |
...
| D-optimality | maximise $det\left(\mathbf{X}^\top\mathbf{X}\right)$ |
| E-optimality | maximise the minimum eigenvalue of $\mathbf{X}^\top\mathbf{X}$ |
...
library(tidyverse)
tibble(block = rep(1:3, 2),
       unit = 1:6,
       trt = c("A", "A", "B", "B", "C", "C"),
       response = c(10.1, 9.8, 8.8, 8.8, 7.4, 7.5)) %>% 
  knitr::kable(caption = "A dummy experimental data. Is the treatment replicated here?") %>% 
...
Above are statistical perspectives for the desired number of replications. In practice, the number of replications are also determined by non-statistical reasons. E.g., there are only limited number of treatments in stock; not enough subjects that qualify for treatments, time constrains and so on.
